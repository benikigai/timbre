 ...
 */
export function AppShell({ state, scoutConnected, runConnected, leftPanel, centerContent, overlays }: AppShellProps) {
  return (
    <div className="min-h-screen bg-[color:var(--color-bg)] text-[color:var(--color-ink)] flex flex-col">
      <Header state={state} scoutConnected={scoutConnected} runConnected={runConnected} />
      <main className="flex-1 grid grid-cols-1 lg:grid-cols-[280px_minmax(0,1fr)] gap-0">
